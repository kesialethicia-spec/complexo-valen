...
import {
  ...
  updateValenbenPageSettings,
  type ValenbenPageSettings,
} from "@/lib/valenben-settings-api";
import { ImageUploadField } from "@/components/admin/ImageUploadField";

export const Route = createFileRoute("/admin/pagina-valenben")({
  component: AdminValenbenPage,
});

const GALLERY_SLOTS = 6;

function AdminValenbenPage() {
  ...
  const [form, setForm] = useState<ValenbenPageSettings | null>(null);
  ...
  const update = <K extends keyof ValenbenPageSettings>(k: K, v: ValenbenPageSettings[K]) =>
    setForm((f) => (f ? { ...f, [k]: v } : f));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await updateValenbenPageSettings({
        ...form,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-display font-bold">Página ValenBen</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Gerencie as imagens e links da página ValenBen Super Troca de Óleo.
          </p>
        </div>
        <Link
          to="/servicos/valenben-super-troca-de-oleo"
          target="_blank"
          ...
          <Section title="Imagens principais" description="Campos vazios usam imagens padrão elegantes.">
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <ImageUploadField
                ...
                hint="Faixa da galeria — foto ampla da equipe."
